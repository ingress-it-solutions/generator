...
class LicMan
{
    ...
    public function checkSettings()
    {
        ...
        if (empty(config('lmconfig.LM_PRODUCT_KEY'))) {
            //invalid encryption salt
            $notifications_array[] = config(
                'lmconfig.LM_CORE_NOTIFICATION_INVALID_PRODUCT_KEY'
            );
        }

        if (empty(config('lmconfig.LM_API_KEY'))) {
            //invalid License Manager Installer API Key
            $notifications_array[] = config(
                'lmconfig.LM_CORE_NOTIFICATION_MISSING_INSTALL_API_KEY'
            );
        }

        if (empty(config('lmconfig.LM_ROOT_URL'))) {
            //invalid License Manager server URL
            $notifications_array[] = config(
                'lmconfig.LM_CORE_NOTIFICATION_INVALID_ROOT_URL'
            );
        }

        if (empty(config('lmconfig.LM_PRODUCT_ID'))) {
            //invalid product ID
            $notifications_array[] = config(
                'lmconfig.LM_CORE_NOTIFICATION_INVALID_PRODUCT_ID'
            );
        }
        ...
        if ($file_headers === false || $file_headers[0] === null) {
            $notifications_array[] = config(
                'lmconfig.LM_NOTIFICATION_NO_CONNECTION'
            );
        }
        if (!$this->validateNumberOrRange(config('lmconfig.LM_DAYS'), 1, 365)) {
            //invalid verification period
            $notifications_array[] = config(
                'lmconfig.LM_CORE_NOTIFICATION_INVALID_VERIFICATION_PERIOD'
            );
        }
    ...
    public function generateScriptSignature(
        $ROOT_URL,
        $CLIENT_EMAIL,
        $LICENSE_CODE
    ) {
    ...
    public function customEncrypt($string, $key)
    {
        ...
        if (!empty($string) && !empty($key)) {
            ...
            $encrypted_string = openssl_encrypt(
                ...
                0,
                $iv
            ); //encrypt the string using AES 256 encryption in CBC mode using encryption key and initialization vector
    ...
    public function parseServerNotifications(
        $content_array,
        $ROOT_URL,
        $CLIENT_EMAIL,
        $LICENSE_CODE
        ...
        if (!empty($content_array)) {
            ...
            if (
                empty($body->notification_server_signature) &&
                $body->notification_case === 'notification_invalid_signature'
            ) {
                $notifications_array['notification_case'] =
                    'notification_invalid_signature';
                $notifications_array['notification_text'] = config(
                    'lmconfig.LM_NOTIFICATION_INVALID_SIGNATURE'
                );
            } elseif (
                !empty($body->notification_server_signature) &&
                $this->verifyServerSignature(
                    $body->notification_server_signature,
                    $ROOT_URL,
                    $CLIENT_EMAIL,
                    $LICENSE_CODE
                )
    ...
    public function getRawDomain($url)
    {
        ...
        if (!empty($url)) {
            ...
            if (empty($url_array['scheme'])) {
                //in case no scheme was provided in url, it will be parsed incorrectly. add http:// and re-parse
                $url = 'http://' . $url;
                $url_array = parse_url($url);
            }
    ...
    public function verifyServerSignature(
    ...
    private static function validateNumberOrRange(
        $number,
        $min_value,
        $max_value = INF
    ) {
        ...
        if (
            filter_var($number, FILTER_VALIDATE_INT) === 0 ||
        ...
        if (stristr($number, '-')) {
            ...
            if (
                filter_var($numbers_array[0], FILTER_VALIDATE_INT) === 0 ||
                (!filter_var($numbers_array[0], FILTER_VALIDATE_INT) ===
                    false &&
                    filter_var($numbers_array[1], FILTER_VALIDATE_INT) === 0) ||
                !filter_var($numbers_array[1], FILTER_VALIDATE_INT) === false
            ) {
                if (
                    $numbers_array[0] >= $min_value &&
                    $numbers_array[1] <= $max_value &&
    ...
    public function getDaysBetweenDates($date_from, $date_to)
    {
        ...
        if (
            $this->verifyDateTime($date_from, 'Y-m-d') &&
            $this->verifyDateTime($date_to, 'Y-m-d')
        ) {
    ...
    public function checkDate($data)
    {
        if ($data['lastCheckedDate']) {
            if (
                Carbon::now() >
                    Carbon::createFromFormat('Y-m-d', $data['lastCheckedDate'])
                        ->addDays(config('lmconfig.LM_DAYS'))
                        ->toDateTimeString() &&
                Carbon::createFromFormat('Y-m-d', $data['lastCheckedDate'])
                    ->addDays(config('lmconfig.LM_DAYS'))
                    ->toDateTimeString() < 8
    ...
    public function checkPersistence()
    {
        ...
        if (
            File::exists(storage_path('app/license.lic')) &&
            File::exists(storage_path('app/public.key'))
        ) {
            ...
            if (
                !empty($data['rootUrl']) &&
                !empty($data['licenseKey']) &&
                !empty($data['lastCheckedDate']) &&
                !empty($data['supportDate'])
            ) {
                //do further check only if essential variables are valid
                $LCD = $data['lastCheckedDate']; //decrypt $LCD value for easier data check

                $verify = $output;

                if ($data['rootUrl'] != env('APP_URL') . '/') {
                    //invalid script url
                    $error_detected = 1;
                }

                if (!$this->verifyDateTime($LCD, 'Y-m-d')) {
                    //last check date is invalid
                    $error_detected = 1;
                }
        ...
        if (
            $cracking_detected == 1 &&
            config('lmconfig.LM_DELETE_CRACKED') == true
        ) {
            //delete user data
    ...
    public function checkData($MYSQLI_LINK = null)
    {
        ...
        if (
            ...
            //do further check only if essential variables are valid
            $LCD = $this->customDecrypt(
                $LCD,
                config('installer.PRODUCT_KEY') . $INSTALLATION_KEY
            ); //decrypt $LCD value for easier data check
            $LRD = $this->customDecrypt(
                $LRD,
                config('installer.PRODUCT_KEY') . $INSTALLATION_KEY
            ); //decrypt $LRD value for easier data check
            ...
            if (
                filter_var($this->getCurrentUrl(), FILTER_VALIDATE_URL) &&
                stristr(
                    $this->getRootUrl($this->getCurrentUrl(), 1, 1, 0, 1),
                    $this->getRootUrl("$ROOT_URL/", 1, 1, 0, 1)
                ) === false
            ) {
                //script is opened via browser (current_url set), but current_url is different from value in database
                $error_detected = 1;
            }
            ...
            if (
                ...
                //invalid installation hash (value - $ROOT_URL, $CLIENT_EMAIL AND $LICENSE_CODE encrypted with sha256)
                $error_detected = 1;
            }
            ...
            if (
                ...
                !password_verify(
                    ...
                    $this->customDecrypt(
                        $INSTALLATION_KEY,
                        config('installer.PRODUCT_KEY') . $ROOT_URL
                    )
                )
            ) {
                //invalid installation key (value - current date ("Y-m-d") encrypted with password_hash and then encrypted with custom function (salt - $ROOT_URL). Put simply, it's LRD value, only encrypted different way)
                $error_detected = 1;
            }
            ...
            if (!$this->verifyDateTime($LRD, 'Y-m-d')) {
                //last run date is invalid
                $error_detected = 1;
            }

            //check for possible cracking attempts - starts
            if (
                $this->verifyDateTime($LCD, 'Y-m-d') &&
                $LCD > date('Y-m-d', strtotime('+1 day'))
            ) {
                //last check date is VALID, but higher than current date (someone manually decrypted and overwrote it or changed system time back). Allow 1 day difference in case user changed his timezone and current date went 1 day back
                $error_detected = 1;
                $cracking_detected = 1;
            }

            if (
                $this->verifyDateTime($LRD, 'Y-m-d') &&
                $LRD > date('Y-m-d', strtotime('+1 day'))
            ) {
                //last run date is VALID, but higher than current date (someone manually decrypted and overwrote it or changed system time back). Allow 1 day difference in case user changed his timezone and current date went 1 day back
                $error_detected = 1;
                $cracking_detected = 1;
            }
            ...
            if (
                ...
                //last check date and last run date is VALID, but LCD is higher than LRD (someone manually decrypted and overwrote it or changed system time back)
                $error_detected = 1;
                $cracking_detected = 1;
            }
            if (
                $this->verifyDateTime($LCD, 'Y-m-d') &&
                $this->verifyDateTime($LRD, 'Y-m-d') &&
                $LRD > date('Y-m-d', strtotime('+7 day')) &&
                $LCD > date('Y-m-d', strtotime('+7 day'))
            ) {
                //last check date and last run date is VALID, but LCD is higher than LRD (someone manually decrypted and overwrote it or changed system time back)
                $error_detected = 1;
                $cracking_detected = 1;
            ...
            if (
                $cracking_detected == 1 &&
                config('lmconfig.LM_DELETE_CRACKED') == 'YES'
            ) {
                //delete user data
                $this->deleteData($MYSQLI_LINK);
            }
    ...
    public function installLicense($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
    {
        ...
        if (empty($apl_core_notifications)) {
            //only continue if script is properly configured

            if ($this->getLicenseData(true) == 1) {
                //license already installed

                $notifications_array['notification_case'] =
                    'notification_already_installed';
                $notifications_array['notification_text'] = config(
                    'lmconfig.LM_NOTIFICATION_SCRIPT_ALREADY_INSTALLED'
                );
            } else {
                //license not yet installed, do it now
                ...
                $post_info =
                    'product_id=' .
                    rawurlencode(config('lmconfig.LM_PRODUCT_ID')) .
                    '&client_email=' .
                    rawurlencode($CLIENT_EMAIL) .
                    '&product_key=' .
                    rawurlencode(config('lmconfig.LM_PRODUCT_KEY')) .
                    '&license_code=' .
                ...
                $content_array = $this->customPost(
                    config('lmconfig.LM_ROOT_URL') . '/api/license/install',
                    $post_info,
                    $ROOT_URL
                );
                // dd($content_array);
                ...
                if (
                    substr(
                        $content_array['body'],
                        0,
                        strlen('Your IP Address is not whitelisted.')
                    ) === 'Your IP Address is not whitelisted.'
                ) {
                    $notifications_array['notification_case'] =
                        'notification_api_not_whitelist';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_API_WHITELIST_ISSUE'
                    );
                } elseif (
                    substr(
                        $content_array['body'],
                        0,
                        strlen('Invalid API key')
                    ) === 'Invalid API key'
                ) {
                    $notifications_array['notification_case'] =
                        'notification_api_invalid';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_INVALID_API'
                    );
                } elseif (
                    substr(
                        $content_array['body'],
                        0,
                        strlen('No valid API key')
                    ) === 'No valid API key'
                ) {
                    $notifications_array['notification_case'] =
                        'notification_api_not_whitelist';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_API_KEY_MISSING'
                    );
                } else {
                    $notifications_array = $this->parseServerNotifications(
                        $content_array,
                        $ROOT_URL,
                        $CLIENT_EMAIL,
                        $LICENSE_CODE
                    ); //process response from Auto PHP Licenser server
                }
                ...
                if (
                    ...
                    $INSTALLATION_KEY = $this->customEncrypt(
                        password_hash(date('Y-m-d'), PASSWORD_DEFAULT),
                        config('lmconfig.LM_PRODUCT_KEY') . $ROOT_URL
                    ); //generate $INSTALLATION_KEY first because it will be used as salt to encrypt LCD and LRD!!!
                    $LCD = $this->customEncrypt(
                        date(
                            'Y-m-d',
                            strtotime(
                                '-' . config('lmconfig.LM_DAYS') . ' days'
                            )
                        ),
                        config('lmconfig.LM_PRODUCT_KEY') . $INSTALLATION_KEY
                    ); //license will need to be verified right after installation
                    $LRD = $this->customEncrypt(
                        date('Y-m-d'),
                        config('lmconfig.LM_PRODUCT_KEY') . $INSTALLATION_KEY
                    );
                    ...
                    $post_info =
                        ...
                        rawurlencode(config('lmconfig.LM_PRODUCT_ID')) .
                        '&siteId=' .
                        rawurlencode($arrayData->returnVariables->siteId) .
                        '&product_key=' .
                        rawurlencode(config('lmconfig.LM_PRODUCT_KEY')) .
                        '&client_email=' .
                        rawurlencode($CLIENT_EMAIL) .
                    ...
                    $pubKey = $this->getMyKey(
                        config('lmconfig.LM_ROOT_URL') . '/api/license/key',
                        $post_info,
                        $ROOT_URL
                    );

                    //  dd($pubKey);
                    $notifications_key = $this->parseServerNotifications(
                        $pubKey,
                        $ROOT_URL,
                        $CLIENT_EMAIL,
                        $LICENSE_CODE
                    ...
                    Storage::put(
                        'public.key',
                        $notifications_key['notification_data']->pubKey
                    );
                    Storage::put(
                        'license.lic',
                        $notifications_key['notification_data']->licenseVal
                    );
                } elseif (
                    $notifications_array['notification_case'] ==
                    'notification_invalid_signature'
                    ...
                    $notifications_array['notification_case'] =
                        'notification_license_expired';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_LICENSE_EXPIRED_PERIOD'
                    );
                    $notifications_array['notification_data'] = [];
                } elseif (
                    $notifications_array['notification_case'] ==
                    ...
                    $notifications_array['notification_case'] =
                        'notification_invalid_ip';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_INVALID_DNS'
                    );
                    $notifications_array['notification_data'] = [];
                } elseif (
                    $notifications_array['notification_case'] ==
                    ...
                    $notifications_array['notification_case'] =
                        'notification_client_not_found';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_CLIENT_NOT_FOUND'
                    );
                    $notifications_array['notification_data'] = [];
                } elseif (
                    $notifications_array['notification_case'] ==
    ...
    public function verifyLicense($rootUrl, $forceCheck = false)
    {
        ...
        if (empty($licman_core_notifications)) {
            //only continue if script is properly configured
            if (File::exists(storage_path('app/public.key'))) {
                $public = File::get(storage_path('app/public.key'));
            } else {
                $notifications_array['notification_case'] =
                    'notification_key_missing';
                $notifications_array['notification_text'] = config(
                    'lmconfig.LM_CORE_NOTIFICATION_LICENSE_KEYFILE_MISSING'
                );
            }
            ...
            if (File::exists(storage_path('app/license.lic'))) {
                ...
                $notifications_array['notification_case'] =
                    'notification_key_missing';
                $notifications_array['notification_text'] = config(
                    'lmconfig.LM_CORE_NOTIFICATION_LICENSE_FILE_MISSING'
                );
            }
            ...
            if (!empty($data)) {
                if ($this->checkDate($data) || $forceCheck) {
                    ...
                    if (
                        $data['productKey'] == config('lmconfig.LM_PRODUCT_KEY')
                    ) {
                        if (
                            !empty($data['rootUrl']) &&
                            !empty($data['clientEmail']) &&
                            !empty($data['licenseKey'])
                        ) {
                            $INSTALLATION_HASH = hash(
                                'sha256',
                            ...
                            $post_info =
                                ...
                                rawurlencode(config('lmconfig.LM_PRODUCT_ID')) .
                                '&siteId=' .
                                rawurlencode($data['siteId']) .
                                '&product_key=' .
                                rawurlencode(
                                    config('lmconfig.LM_PRODUCT_KEY')
                                ) .
                                '&client_email=' .
                                rawurlencode($data['clientEmail']) .
                                '&license_code=' .
                                rawurlencode($data['licenseKey']) .
                                '&root_url=' .
                                rawurlencode($rootUrl) .
                                '&installation_hash=' .
                                rawurlencode($INSTALLATION_HASH) .
                                '&license_signature=' .
                                rawurlencode(
                            ...
                            if (
                                $pubKey['body'] ===
                                    'Your IP Address is not whitelisted.' ||
                                $pubKey['body'] === 'Invalid API key' ||
                                $pubKey['body'] === 'No valid API key'
                            ) {
                                $notifications_array['notification_case'] =
                                    'notification_api_not_whitelist';
                                $notifications_array[
                            ...
                            //  dd((array)$notifications_key['notification_data']);
                            if (!empty($notifications_key)) {
                                if (
                                    !empty(
                                        $notifications_key['notification_data']
                                    ) &&
                                    array_key_exists(
                                        'licenseVal',
                                        (array) $notifications_key[
                                            'notification_data'
                                        ]
                                    )
                                ) {
                                    ...
                                    if (
                                        ...
                                        $notifications_array[
                                            ...
                                            'notification_license_support_expired';
                                        $notifications_array[
                                            'notification_text'
                                        ] = config(
                                            'lmconfig.LM_CORE_NOTIFICATION_LICENSE_SUPPORT_EXPIRED'
                                        );
                                    }

                                    if (
                                        $licenseVal['productId'] !=
                                        config('lmconfig.LM_PRODUCT_ID')
                                    ) {
                                        // invalid license
                                        $notifications_array[
                                        ...
                                        $notifications_array[
                                            ...
                                            'lmconfig.LM_NOTIFICATION_LICENSE_CORRUPTED'
                                        );
                                        if (
                                            config('lmconfig.LM_DELETE_CRACKED')
                                        ) {
                                    ...
                                    if ($licenseVal['cancelDate']) {
                                        ...
                                        $notifications_array[
                                            ...
                                            'lmconfig.LM_CORE_NOTIFICATION_LICENSE_SUSPENDED'
                                        );
                                    }

                                    if (
                                        $licenseVal['productKey'] !=
                                        config('lmconfig.LM_PRODUCT_KEY')
                                    ) {
                                        // invalid license
                                        $notifications_array[
                                    ...
                                    if (
                                        $licenseVal['rootUrl'] != $rootUrl &&
                                        $licenseVal['installLimit'] > 1
                                    ) {
                                        // invalid domain for installation.LM_CORE_NOTIFICATION_INVALID_ROOT_URL
                                        $notifications_array[
                                            'notification_case'
                                        ] = 'notification_invalid_url';
                                        $notifications_array[
                                            'notification_text'
                                        ] = config(
                                            'lmconfig.LM_CORE_NOTIFICATION_INVALID_ROOT_URL'
                                    ...
                                    $notifications_array['notification_case'] =
                                        $notifications_key['notification_text'];
                                    $notifications_array['notification_text'] =
                                        $notifications_key['notification_text'];
                    ...
                    if (
                        $data['productId'] != config('lmconfig.LM_PRODUCT_ID')
                    ) {
                        // invalid license
                        $notifications_array['notification_case'] =
                            'notification_license_corrupted';
                    ...
                    if ($data['installLimit'] < $data['totalInstall']) {
                        ...
                        $notifications_array['notification_text'] = config(
                            'lmconfig.LM_NOTIFICATION_LICENSE_OCCUPIED'
                        );
                    }
    ...
    public function uninstallLicense($rootUrl)
    {
        ...
        if (empty($apl_core_notifications)) {
            ...
            if (!empty($data)) {
                ...
                if (
                    !array_key_exists('notification_case', $notifications_array)
                ) {
                    $notifications_array['notification_case'] =
                        'notification_license_ok';
                    $notifications_array['notification_text'] = null;
                    ...
                    $INSTALLATION_HASH = hash(
                        'sha256',
                        $rootUrl . $data['clientEmail'] . $data['licenseKey']
                    ); //generate hash
                    $post_info =
                        'product_id=' .
                        rawurlencode(config('lmconfig.LM_PRODUCT_ID')) .
    ...
    public function updateLicense($rootUrl)
    {
        $notifications_array = [];
        $apl_core_notifications = $this->checkSettings(); //check core settings
    ...
    public function checkLicenseUpdate($rootUrl)
    {
        $notifications_array = [];
        $apl_core_notifications = $this->checkSettings(); //check core settings
            ...
            if (!empty($data)) {
                ...
                if ($data['updateDate'] || !empty($data['updateDate'])) {
                    if ($data['updateDate'] > Carbon::now()->format('Y-m-d')) {
                        // license cancelled / suspended.LM_CORE_NOTIFICATION_LICENSE_SUSPENDED
                        $notifications_array['notification_case'] =
                            'notification_license_update_expired';
                        $notifications_array['notification_text'] = config(
                            'lmconfig.LM_CORE_NOTIFICATION_LICENSE_UPDATE_EXPIRED'
                        );
                    }
                } else {
                    $notifications_array['notification_case'] =
                        'notification_license_no_update';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_LICENSE_NO_UPDATE'
                    );
                }
    ...
    public function checkLicenseSupport($rootUrl)
    {
        $notifications_array = [];
        $apl_core_notifications = $this->checkSettings(); //check core settings
        ...
        if (empty($apl_core_notifications)) {
            ...
            if (!empty($data)) {
                ...
                if ($data['supportDate'] || !empty($data['supportDate'])) {
                    if ($data['supportDate'] < Carbon::now()) {
                        //support expired.LM_CORE_NOTIFICATION_LICENSE_SUPPORT_EXPIRED
                        $notifications_array['notification_case'] =
                            'notification_license_support_expired';
                    ...
                    $notifications_array['notification_case'] =
                        'notification_license_no_support';
                    $notifications_array['notification_text'] = config(
                        'lmconfig.LM_CORE_NOTIFICATION_LICENSE_NO_SUPPORT'
                    );
                }
    ...
    public function checkProductUpdate($rootUrl, $product_version)
    {
        $notifications_array = [];
        $apl_core_notifications = $this->checkSettings(); //check core settings
    ...
    public function getRootUrl(
        ...
        if (filter_var($url, FILTER_VALIDATE_URL)) {
            ...
            if ($remove_last_slash == 1) {
                //remove / from the end of URL if it exists
                while (substr($url, -1) == '/') {
                    //use cycle in case URL already contained multiple // at the end
                    $url = substr($url, 0, -1);
                }
            }
        }
    ...
    public function deleteData($MYSQLI_LINK = null)
    {
        if (isset($_SERVER['DOCUMENT_ROOT'])) {
            //god mode enabled, delete everything from document root directory (usually httpdocs or public_html). god mode might not be available for IIS servers that don't always set $_SERVER['DOCUMENT_ROOT']
    ...
    public function customDecrypt($string, $key)
    {
        ...
        if (!empty($string) && !empty($key)) {
            ...
            if (stristr($string, '::')) {
                //unique separator "::" found, most likely it's valid encrypted string
                ...
                if (!empty($string_iv_array) && count($string_iv_array) == 2) {
                    //proper $string_iv_array should contain exactly two values - $encrypted_string and $iv
