    ...
    public function installLicense($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE) {
        ...
        if (empty($apl_core_notifications)) { //only continue if script is properly configured
            ...
            if ($this->getLicenseData(true) == 1) { //license already installed
                ...
                if (substr($content_array['body'], 0, strlen('Your IP Address is not whitelisted.')) === 'Your IP Address is not whitelisted.') {
                ...
                } elseif(substr($content_array['body'], 0, strlen('Invalid API key')) === 'Invalid API key') {
                    $notifications_array['notification_case'] = "notification_api_invalid";
                    $notifications_array['notification_text'] = config('lmconfig.LM_CORE_NOTIFICATION_INVALID_API');
                }elseif(substr($content_array['body'], 0, strlen('No valid API key')) === 'No valid API key') {
                    $notifications_array['notification_case'] = "notification_api_not_whitelist";
                    $notifications_array['notification_text'] = config('lmconfig.LM_CORE_NOTIFICATION_API_KEY_MISSING');
                } else {
                ...
                if ($this->checkDate($data) || $forceCheck) {
                    ...
                    if ($data['productKey'] == config('lmconfig.LM_PRODUCT_KEY')) {
                        ...
                        if (!empty($data['rootUrl']) && !empty($data['clientEmail']) && !empty($data['licenseKey'])) {
                            ...
                            $pubKey = $this->getMyKey(config('lmconfig.LM_ROOT_URL') . "/api/license/verify", $post_info, $rootUrl);
                            //dd($pubKey);
                            ...
                            if(!empty($notifications_key)) {
                                ...
                                if (!empty($notifications_key['notification_data']) && array_key_exists('licenseVal', (array)$notifications_key['notification_data'])) {
                                    // dd(1);
        ...
        if (empty($apl_core_notifications)) {//only continue if script is properly configured
            ...
            if (!empty($data)) {
                ...
                if($data['supportDate'] || !empty($data['supportDate'])) {
                    ...
                    $notifications_array['notification_case'] = "notification_license_no_support";
                    $notifications_array['notification_text'] = config('lmconfig.LM_CORE_NOTIFICATION_LICENSE_NO_SUPPORT');
                }
            ...
            if (!empty($data)) {
                ...
                if(!array_key_exists('notification_case', $notifications_array)){
                    ...
                    if ($pubKey['body'] === 'Your IP Address is not whitelisted.' || $pubKey['body'] === 'Invalid API key' || $pubKey['body'] === 'No valid API key') {
                        ...
                        if($notifications_key['notification_case'] === 'notification_license_expired') {
                            $notifications_array['notification_case'] = "notification_license_expired";
                            $notifications_array['notification_text'] = config('lmconfig.LM_CORE_NOTIFICATION_LICENSE_EXPIRED_PERIOD');
                        } elseif($notifications_key['notification_case'] === 'notification_site_not_found') {
                            $notifications_array['notification_case'] = "notification_site_not_found";
                            $notifications_array['notification_text'] = config('lmconfig.LM_CORE_NOTIFICATION_API_WHITELIST_ISSUE');

                        } elseif($notifications_key['notification_case'] === 'notification_version_not_found') {
                            $notifications_array['notification_case'] = "notification_version_not_found";
                            $notifications_array['notification_text'] = config('lmconfig.LM_CORE_NOTIFICATION_VERSION_NOT_FOUND');
                        }else {
                            $notifications_array['notification_case'] = 'notification_license_ok';
                            $notifications_array['notification_text'] = null;
                            $notifications_array['notification_data'] = $notifications_key['notification_data'];
