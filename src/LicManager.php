...
//ALL THE FUNCTIONS IN THIS FILE START WITH licMan TO PREVENT DUPLICATED NAMES WHEN AUTO PHP LICENSER SOURCE FILES ARE INTEGRATED INTO ANY SCRIPT


//encrypt text with custom key
public static function licManCustomEncrypt($string, $key)
    {
...
public static function licManCustomDecrypt($string, $key)
    {
...
public static function licManValidateNumberOrRange($number, $min_value, $max_value=INF)
    {
    $result=false;
...
public static function licManValidateRawDomain($url)
    {
    $result=false;
...
public static function licManGetCurrentUrl($remove_last_slash=null)
    {
    $current_url=null;
...
public static function licManGetRawDomain($url)
    {
    $raw_domain=null;
...
public static function licManGetRootUrl($url, $remove_scheme, $remove_www, $remove_path, $remove_last_slash)
    {
...
public static function licManCustomPost($url, $post_info=null, $refer=null)
    {
    ...
    if (filter_var($url, FILTER_VALIDATE_URL) && !empty($post_info))
        ...
        //this public static function is called by curl for each header received - https://stackoverflow.com/questions/9183178/can-php-curl-retrieve-response-headers-and-body-in-a-single-request
...
public static function licManVerifyDateTime($datetime, $format)
    {
    $result=false;
...
public static function licManGetDaysBetweenDates($date_from, $date_to)
    {
    $number_of_days=0;

    if ($this->licManVerifyDateTime($date_from, "Y-m-d") && $this->licManVerifyDateTime($date_to, "Y-m-d"))
        {
...
public static function licManParseXmlTags($content, $tag_name)
    {
    $parsed_value=null;
...
public static function licManParseServerNotifications($content_array, $ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
    {
    $notifications_array=array();

    if (!empty($content_array)) //response received, validate it
        {
        if (!empty($content_array['headers']['notification_server_signature']) && $this->licManVerifyServerSignature($content_array['headers']['notification_server_signature'], $ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)) //response valid
            {
...
public static function licManGenerateScriptSignature($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
    {
    $script_signature=null;
    $root_ips_array=gethostbynamel($this->licManGetRawDomain(APL_ROOT_URL));
...
public static function licManVerifyServerSignature($notification_server_signature, $ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
    {
    $result=false;
    $root_ips_array=gethostbynamel($this->licManGetRawDomain(APL_ROOT_URL));
...
public static function licManCheckSettings()
    {
    $notifications_array=array();
    ...
    if (!empty(APL_ROOT_IP) && !in_array(APL_ROOT_IP, gethostbynamel($this->licManGetRawDomain(APL_ROOT_URL)))) //actual IP address of Auto PHP Licenser server doesn't match specified IP address
        {
    ...
    if (defined("APL_ROOT_NAMESERVERS") && !empty(APL_ROOT_NAMESERVERS)) //check if actual nameservers of Auto PHP Licenser server domain match specified nameservers (use "defined" to check if nameservers are set because APL_ROOT_NAMESERVERS is commented by default to prevent errors in PHP<7)
        ...
        $dns_records_array=dns_get_record($this->licManGetRawDomain(APL_ROOT_URL), DNS_NS);
...
public static function licManParseLicenseFile()
    {
...
public static function licManGetLicenseData($MYSQLI_LINK=null)
    {
    $settings_row=array();

    if (APL_STORAGE=="DATABASE") //license stored in database (use @ before mysqli_ public static function to prevent errors when public static function is executed by licManInstallLicense function)
        {
...
public static function licManCheckConnection()
    {
    $notifications_array=array();
...
public static function licManCheckData($MYSQLI_LINK=null)
    {
    $error_detected=0;
    $cracking_detected=0;
    $data_check_result=false;

    extract($this->licManGetLicenseData($MYSQLI_LINK)); //get license data
    ...
    if (!empty($ROOT_URL) && !empty($INSTALLATION_HASH) && !empty($INSTALLATION_KEY) && !empty($LCD) && !empty($LRD)) //do further check only if essential variables are valid
        ...
        if (filter_var($this->licManGetCurrentUrl(), FILTER_VALIDATE_URL) && stristr($this->licManGetRootUrl($this->licManGetCurrentUrl(), 1, 1, 0, 1), licManGetRootUrl("$ROOT_URL/", 1, 1, 0, 1))===false) //script is opened via browser (current_url set), but current_url is different from value in database
            {
            $error_detected=1;
            }
        ...
        if (empty($INSTALLATION_KEY) || !password_verify($LRD, licManCustomDecrypt($INSTALLATION_KEY, APL_SALT.$ROOT_URL))) //invalid installation key (value - current date ("Y-m-d") encrypted with password_hash and then encrypted with custom public static function (salt - $ROOT_URL). Put simply, it's LRD value, only encrypted different way)
            {
            $error_detected=1;
            }
        ...
        if ($this->licManVerifyDateTime($LCD, "Y-m-d") && $LCD>date("Y-m-d", strtotime("+1 day"))) //last check date is VALID, but higher than current date (someone manually decrypted and overwrote it or changed system time back). Allow 1 day difference in case user changed his timezone and current date went 1 day back
            {
            $error_detected=1;
            $cracking_detected=1;
            }

        if ($this->licManVerifyDateTime($LRD, "Y-m-d") && $LRD>date("Y-m-d", strtotime("+1 day"))) //last run date is VALID, but higher than current date (someone manually decrypted and overwrote it or changed system time back). Allow 1 day difference in case user changed his timezone and current date went 1 day back
            {
            $error_detected=1;
            $cracking_detected=1;
            }

        if ($this->licManVerifyDateTime($LCD, "Y-m-d") && $this->licManVerifyDateTime($LRD, "Y-m-d") && $LCD>$LRD) //last check date and last run date is VALID, but LCD is higher than LRD (someone manually decrypted and overwrote it or changed system time back)
            {
            $error_detected=1;
            $cracking_detected=1;
            }
...
public static function licManVerifyEnvatoPurchase($LICENSE_CODE=null)
    {
    $notifications_array=array();
...
//install license
public static function licManInstallLicense($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE, $MYSQLI_LINK=null)
    {
    $notifications_array=array();
    ...
    if (empty($apl_core_notifications)) //only continue if script is properly configured
        {
        if (!empty($this->licManGetLicenseData($MYSQLI_LINK)) && is_array($this->licManGetLicenseData($MYSQLI_LINK))) //license already installed
            {
            ...
            $post_info="product_id=".rawurlencode(APL_PRODUCT_ID)."&client_email=".rawurlencode($CLIENT_EMAIL)."&license_code=".rawurlencode($LICENSE_CODE)."&root_url=".rawurlencode($ROOT_URL)."&installation_hash=".rawurlencode($INSTALLATION_HASH)."&license_signature=".rawurlencode($this->licManGenerateScriptSignature($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE));
...
//verify license
public static function licManVerifyLicense($MYSQLI_LINK=null, $FORCE_VERIFICATION=0)
    {
    $notifications_array=array();
    $update_lrd_value=0;
    $update_lcd_value=0;
    ...
    if (empty($apl_core_notifications)) //only continue if script is properly configured
        {
        if ($this->licManCheckData($MYSQLI_LINK)) //only continue if license is installed and properly configured
            {
            extract($this->licManGetLicenseData($MYSQLI_LINK)); //get license data

            if ($this->licManGetDaysBetweenDates($this->licManCustomDecrypt($LCD, APL_SALT.$INSTALLATION_KEY), date("Y-m-d"))<APL_DAYS && $this->licManCustomDecrypt($LCD, APL_SALT.$INSTALLATION_KEY)<=date("Y-m-d") && $this->licManCustomDecrypt($LRD, APL_SALT.$INSTALLATION_KEY)<=date("Y-m-d") && $FORCE_VERIFICATION==0) //the only case when no verification is needed, return notification_license_ok case, so script can continue working
                {
            ...
            if ($this->licManCustomDecrypt($LRD, APL_SALT.$INSTALLATION_KEY)<date("Y-m-d")) //used to make sure database gets updated only once a day, not every time script is executed. do it BEFORE new $INSTALLATION_KEY is generated
                {
                $update_lrd_value=1;
                }
...
//verify support
public static function licManVerifySupport($MYSQLI_LINK=null)
    {
    $notifications_array=array();
    $apl_core_notifications=aplCheckSettings(); //check core settings
...
//verify updates
public static function licManVerifyUpdates($MYSQLI_LINK=null)
    {
    $notifications_array=array();
    $apl_core_notifications=aplCheckSettings(); //check core settings
...
//update license
public static function licManUpdateLicense($MYSQLI_LINK=null)
    {
    $notifications_array=array();
    $apl_core_notifications=aplCheckSettings(); //check core settings
...
//uninstall license
public static function licManUninstallLicense($MYSQLI_LINK=null)
    {
    $notifications_array=array();
    $apl_core_notifications=aplCheckSettings(); //check core settings
...
//delete user data
public static function licManDeleteData($MYSQLI_LINK=null)
    {
